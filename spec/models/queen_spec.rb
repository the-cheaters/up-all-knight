  ...
  describe "Queen#valid_move?(destination_x, destination_y)" do
    ...
    context "up" do
      ...
      it "should return true if the square is occupied by a piece of the opposing color" do
        piece = FactoryGirl.create(:piece, x_position: 4, y_position: 6, game_id: game.id, player_id: black_player.id)
        expect(subject).to eq(true)
      end
    ...
    context "left" do
      ...
      it "should return true if the square is occupied by a piece of the opposing color" do
        piece = FactoryGirl.create(:piece, x_position: 0, y_position: 4, game_id: game.id, player_id: black_player.id)
        expect(subject).to eq(true)
      end

      it "should return false if the square is occupied by a piece of the same color" do
        piece = FactoryGirl.create(:piece, x_position: 0, y_position: 4, game_id: game.id, player_id: white_player.id)
        expect(subject).to eq(false)
      end

      it "should return false if the queen is obstructed" do
        piece = FactoryGirl.create(:piece, x_position: 1, y_position: 4, game_id: game.id, player_id: white_player.id)
        expect(subject).to eq(false)
      end
