...
RSpec.describe PiecesController, type: :controller do
  ...
  it "has a 200 status code" do
    xhr :post, :update, game_id: game.id, id: knight.id , piece: { x_position: '2', y_position: '3' }
    expect(response.code).to eq('200')
  end
  
  it "has a 422 status code" do
    xhr :post, :update, game_id: game.id, id: knight.id , piece: { x_position: '2', y_position: '4' }
    expect(response.code).to eq('422')
  end
  
  it "has a 422 status code if passing nil" do
    xhr :post, :update, game_id: game.id, id: knight.id , piece: { x_position: 'nil', y_position: 'nil' }
    expect(response.code).to eq('422')
  end
end
